  ...
  return (
    <div className="flex h-screen w-full">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input className="h-12 border-neutral-300 focus:border-primary focus:border-2 transition-all rounded-none" type="text" placeholder="name@dentsu.com" required value={email} onChange={(e) => setEmail(e.target.value)} disabled={isLoggingIn} />
              </div>
              <div className="space-y-1.5">
            ...
            <Button type="submit" className="w-full h-12 font-bold uppercase tracking-[0.15em] text-xs" disabled={isLoggingIn}>{isLoggingIn ? 'Signing in…' : 'Sign in'}</Button>
          </form>
          <div className="flex flex-col gap-4 pt-6 border-t border-hairline">
             <div className="flex items-center gap-2"><span className="text-xs text-secondary">Need an account?</span><Link href="/register" className="text-xs font-bold uppercase text-brand hover:underline">Request access</Link></div>
      ...
      <div className="hidden lg:block lg:w-1/2 relative bg-ink overflow-hidden">
        ...
        <div className="absolute bottom-12 left-12 right-12"><div className="p-8 border border-white/20 bg-ink/70"><p className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/70 mb-3">AZTEC Control Center</p><h2 className="text-white text-3xl font-black tracking-tight mb-3 normal-case">Client performance, spends, and weekly reviews in one place.</h2><p className="text-white/60 text-xs font-mono tracking-widest uppercase">Ops console for media &amp; performance teams</p></div></div>
      </div>
    </div>
  );
}
